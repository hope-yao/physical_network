...
class DR_RNN:
    ...
    def get_residual(self, y_tp1, y_t, delta_t):
        ...
        c1 = 0.02
        c2 = 0.02
        A = tf.convert_to_tensor([[0, 1, 0, 0, 0, 0],
    ...
    def build_training_model(self):
        ...
        # self.ode_para = tf.Variable(tf.truncated_normal([2, ], mean= 0.5, stddev=0.1), name='ode_para')
        self.ode_para = tf.constant([1., 1., 1.])
        self.weight_w = tf.Variable(tf.truncated_normal([6, ], stddev=0.1), name='weight_w')
    ...
    def training_init(self):
        ...
        self.summary_op_testing = tf.summary.merge([
            ...
            tf.summary.scalar("ODE/ode_para_0", self.ode_para[0]),
            tf.summary.scalar("ODE/ode_para_1", self.ode_para[1]),
            # tf.summary.scalar("ODE/ode_para_2", self.ode_para[2]),
        ])
    ...
    def train_model(self):
        ...
        for epoch in tqdm(range(self.num_epochs)):
            it_per_ep = int(len(self.y_train_data) / self.batch_size)
            for i in range(it_per_ep):
                ...
                if count % 10 == 0:
                    ...
                    self.testing_loss_value = self.sess.run(self.testing_loss, {self.y_true: self.y_test_data[rand_idx],
                                                                                self.delta_t_testing: self.delta_t})
                    print("iter:{}  train_cost: {}  test_cost: {} ".format(count, self.training_loss_value,
                                                                           self.testing_loss_value))
                    training_summary, testing_summary = self.sess.run(
                        [self.summary_op_training, self.summary_op_testing],
    ...
    def extrapolation_in_time(self):
        ...
        plt.plot(np.arange(0, (tt_y.shape[1] - 0.5) * self.delta_t, self.delta_t),
                 self.y_test_all[0, :tt_y.shape[1], idx],
                 'b', label='ODE45')
        plt.legend()
        plt.axvspan(self.num_time_steps * self.delta_t, self.num_time_steps * self.delta_t * 2, facecolor='g',
                    alpha=0.5)
        plt.show()
    ...
    def save_data(self, dist_viz=0):
        ...
        np.save(self.logdir + '/k{}_test_drrnn.npy'.format(k),
                self.sess.run(self.y_pred_testing,
    ...
    def visualization(self, delta_t_training, delta_t_testing, count, dist_viz=0):
        ...
        cc = self.sess.run(self.y_pred_testing,
                           {self.y_true: self.y_test_data[:self.batch_size], self.delta_t_testing: delta_t_testing})
        idx = 1
        fig = plt.figure()
        plt.plot(np.arange(0, self.num_time_steps, 1) * delta_t_training, aa[0, :, idx], 'k',
                 label='k{}_numerical'.format(self.num_layers))
        plt.plot(np.arange(0, self.num_time_steps, 1) * delta_t_training, bb[0, :, idx], 'b',
                 label='k{}_train_drrnn'.format(self.num_layers))
        plt.plot(np.arange(0, self.num_time_steps, 1) * delta_t_testing, cc[0, :, idx], 'r',
                 label='k{}_test_drrnn'.format(self.num_layers))
        plt.legend()
        plt.xlabel('physical time')
        plt.ylabel('y{}'.format(idx))
        ttl_name = 'itr:{:d}  '.format(count) + 'train_err:{0:.4e}  '.format(
            self.training_loss_value) + 'test_err:{0:4e}'.format(self.testing_loss_value)
        plt.title(ttl_name)
        # plt.axis([0,10,0,1.2])
        # plt.show()
        ...
        if dist_viz:
            plt.figure()
            y = sio.loadmat('./data/3dof_sys.mat.mat')['y']
            y2_end = y[:, -1, 1]
