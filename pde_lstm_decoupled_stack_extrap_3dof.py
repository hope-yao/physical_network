...
class PDE_LSTM():
    ...
    def get_data(self):
        ...
        import numpy as np
        aa = sio.loadmat('./data/3dof_sys_60s_sin_dp20.mat')
        data = aa['y'][:500,:501,:]
        if 1:
            new_data = []
            for data_i in data:
        ...
        for i in range(data.shape[0]):
            if i % 10 != 0:
                y_train += [data[i, :self.num_time_steps, :]]
                range_dict['train_min'] += [range_dict['min'][i, :]]
                range_dict['train_max'] += [range_dict['max'][i, :]]
                range_dict['train_init'] += [range_dict['init'][i, :]]
                # y_train_ex += [data[i, self.num_time_steps:, :]]
            else:
                y_test += [data[i, :self.num_time_steps, :]]
                range_dict['test_min'] += [range_dict['min'][i, :]]
                range_dict['test_max'] += [range_dict['max'][i, :]]
                range_dict['test_init'] += [range_dict['init'][i, :]]
                # y_test_ex += [data[i, self.num_time_steps:, :]]
        ...
        for i in range(data.shape[0]):
            if i % 2 != 0:
                ...
                range_dict['train_init'] += [range_dict['init'][i, :]]
                y_train_ex += [data[i, :, :]]
                range_dict['train_ex_min'] += [range_dict['min'][i, :]]
                range_dict['train_ex_max'] += [range_dict['max'][i, :]]
                range_dict['train_ex_init'] += [range_dict['init'][i, :]]
            else:
                y_test += [data[i, :self.num_time_steps, :]]
                range_dict['test_min'] += [range_dict['min'][i, :]]
                range_dict['test_max'] += [range_dict['max'][i, :]]
                range_dict['test_init'] += [range_dict['init'][i, :]]
                y_test_ex += [data[i, :, :]]
                range_dict['test_ex_min'] += [range_dict['min'][i, :]]
                range_dict['test_ex_max'] += [range_dict['max'][i, :]]
                range_dict['test_ex_init'] += [range_dict['init'][i, :]]
        ...
        if 1:
            new_y_test = sio.loadmat('./data/3dof_loading_extrapolate_rn.mat')['y1']
        ...
        data_dict['new_y_test_ex'] = np.copy(new_y_test[:, :2001])
        ...
        train_control =  [[[np.sin(10*t), np.sin(5*t), np.sin(10*t)] for t in range(data_dict['y_train'].shape[1])]] * data_dict['y_train'].shape[0]
        train_control_ex = [[[np.sin(10*t), np.sin(5*t), np.sin(10*t)] for t in range(data_dict['y_train_ex'].shape[1])]] * data_dict['y_train_ex'].shape[0]
        test_control = [[[np.sin(10*t), np.sin(5*t), np.sin(10*t)] for t in range(data_dict['y_test'].shape[1])]] * data_dict['y_test'].shape[0]
        test_control_ex = [[[np.sin(10*t), np.sin(5*t), np.sin(10*t)] for t in range(data_dict['y_test_ex'].shape[1])]] * data_dict['y_test_ex'].shape[0]
        ...
        if 1:
            aa = sio.loadmat('./data/3dof_loading_extrapolate_rn.mat')['u'].transpose()
            new_test_control = [aa[:self.num_time_steps]] * self.batch_size
            new_test_control_ex = [aa[:2001]] * self.batch_size
    ...
    def train_init(self):
        ...
        self.saver.restore(self.sess,
                      '/home/hope-yao/Documents/physical_network/saved_models/YANGYU_RESULT/3DOF/3dof_lstm4/saved_models/3dof_lstm2.ckpt')
