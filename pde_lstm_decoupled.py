...
y_pred = my_lstm(control_pl, y_true_pl[:, 0, :])
...
NEW_TEST_SET = 0
if NEW_TEST_SET:
    y_test = sio.loadmat('/home/hope-yao/Documents/physical_network/data/Boeing747_step_200s_ts01_normalized_case4.mat')['y'][:,:num_time_steps+1, 4:]
    new_data = []
    for data_i in y_test:
        new_data += [data_i[:-1,:] - data_i[1:,:]]
    new_data = np.asarray(new_data)
    new_data_normalized =(new_data - np.expand_dims(np.min(new_data,1),1) ) / np.expand_dims(np.max(new_data,1) - np.min(new_data,1), 1)
    y_test = np.tile(new_data_normalized,(batch_size, 1, 1))
...
if NEW_TEST_SET:
    test_control = [[[1/57.2958, 0]] * 20 + [[0, 0]] * 80] * batch_size
...
for ep_i in range(max_epoch):
    ...
    print(ep_i, np.mean(ave_loss_val_train), np.mean(ave_loss_val_test))
